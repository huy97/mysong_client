    ...
    render() {
        ...
        const columns = [
                ...
                render: (value, record) => (
                    <>
                        <Tag color="green" key={0}>
                            <a href={record.link}>MP3</a>
                        </Tag>
                        {value ? (
                            <Tag color="geekblue" key={1}>
                                <a href={record.mvLink}>MV</a>
                            </Tag>
                        ) : null}
                    </>
                ),
            ...
            },
            {
                ...
                dataIndex: "artists",
                render: (artists) =>
                    artists.map((obj, index) => (
                        <React.Fragment key={index}>
                            <Link to={`/nghe-si/${obj.shortCode}.html`}>
                                {obj.fullName}
                            </Link>
                            ,{" "}
                        </React.Fragment>
            ...
            },
            {
                ...
                render: (categories) =>
                    categories.map((obj, index) => (
                        <React.Fragment key={index}>
                            <Link to={`/the-loai/${obj.shortCode}.html`}>
                                {obj.title}
                            </Link>
                            ,{" "}
                        </React.Fragment>
                ...
                render: (text, record) => (
                    <>
                        ...
                        <Ability roles={[PERMISSION_CODE.DELETE]}>
                            <Popconfirm
                                title="Xác nhận xoá bài hát này?"
                                placement="topLeft"
                                okText="Xoá"
                                cancelText="Huỷ"
                                okType="danger"
