    ...
    render() {
        ...
        const columns = [
            ...
            },
            {
                title: 'Tên nghệ sĩ',
                dataIndex: 'fullName',
                render: (text, record) => <Link to={record.link}>{text} {record.isComposer ? <RiStarSLine /> : null}</Link>
            },
            {
                title: 'Chức danh',
                dataIndex: 'isComposer',
                render: (value) => (value ? <Tag color="green">Ca sĩ - Nhạc sĩ</Tag>  : <Tag color="green">Ca sĩ</Tag>)
            },
            {
                title: 'Ảnh đại diện',
                dataIndex: 'thumbnail',
                ...
                render: (text, record) => (
                    <>
                        ...
                        <Ability roles={[PERMISSION_CODE.DELETE]}>
                            <Popconfirm title="Xác nhận xoá thể loại này?" placement="topLeft" okText="Xoá" cancelText="Huỷ" okType="danger" icon={<QuestionCircleOutlined style = {{ color: 'red' }}/>} onConfirm={(e) => onDelete(e, record)}>
